...
float AccErrorX = 1.42;
float AccErrorY = -2.00;
float AccErrorZ = -2.85;
float GyroErrorX = -0.74;
float GyroErrorY = -1.93;
float GyroErrorZ = -5.88;
...
#define PROP_FR_PIN 2   //m2
#define PROP_FL_PIN 3   //m1
#define PROP_BR_PIN 4   //m3
#define PROP_BL_PIN 5   //m4
...
//      command = String((char*)buf);
      if(buf[0]==127)//ping
      {
        ...
        outBytes[0] = 127;
        outBytes[1] = goHover;
//        Serial.println(outBytes[1]);
        send(outBytes, 2);
      }
      if(buf[0]==1)//hover en
      {
        goHover = buf[1]==2;
        Serial.println(goHover);
      }
//      parse();
    }
    else
...
void send(String str)
{
  ...
  str.toCharArray(message, str.length()+1);
  rf95.send((uint8_t *)message, str.length()+1);
}
void send(uint8_t bytes[], int len)
{
  rf95.send(bytes, len);
}
void failSafe() {
...
void commandMotors() {
  ...
  digitalWrite(PROP_FL_PIN, HIGH);
  digitalWrite(PROP_FR_PIN, HIGH);
  digitalWrite(PROP_BR_PIN, HIGH);
  digitalWrite(PROP_BL_PIN, HIGH);
...
//  digitalWrite(m6Pin, HIGH);
  ...
  while (wentLow < 6 ) { //Keep going until final (6th) pulse is finished, then done
    ...
    if ((m1_command_PWM <= timer - pulseStart) && (flagM1==0)) {
      digitalWrite(PROP_FL_PIN, LOW);
      wentLow = wentLow + 1;
      flagM1 = 1;
    }
    if ((m2_command_PWM <= timer - pulseStart) && (flagM2==0)) {
      digitalWrite(PROP_FR_PIN, LOW);
      wentLow = wentLow + 1;
      flagM2 = 1;
    }
    if ((m3_command_PWM <= timer - pulseStart) && (flagM3==0)) {
      digitalWrite(PROP_BR_PIN, LOW);
      wentLow = wentLow + 1;
      flagM3 = 1;
    }
    if ((m4_command_PWM <= timer - pulseStart) && (flagM4==0)) {
      digitalWrite(PROP_BL_PIN, LOW);
      wentLow = wentLow + 1;
      flagM4 = 1;
    } 
